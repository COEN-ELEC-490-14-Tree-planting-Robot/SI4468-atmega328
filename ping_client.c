...
void main(void)
{
	...
	stdout = &uart_io;
	UCSR0B =  ((1<<RXCIE0)|(1<<RXEN0)|(1 << TXEN0));   //Transmitter Enable: enabled
	UCSR0C = (3 << UCSZ00);
	// Start up
	Si446x_init();
	...
	printf_P(PSTR("Robot starts"));
	while(1)
	{
		if(USART_RxTail!=USART_RxHead){
			if(USART_RxHead>USART_RxTail){
				size = USART_RxHead-USART_RxTail;
				...
				memcpy(buff,&USART_RxBuf[USART_RxTail],size);
				USART_RxTail=0;
				if(size+USART_RxHead<=MAX_PACKET_SIZE){
					memcpy(&buff[size],&USART_RxBuf[0],USART_RxHead);
					size+=USART_RxHead;
					USART_RxTail=USART_RxHead;
				}
			}
			
			Si446x_TX((uint8_t*)buff,size, CHANNEL, SI446X_STATE_RX);
			puts_P(PSTR("Data Sent"));
			uint8_t success;
